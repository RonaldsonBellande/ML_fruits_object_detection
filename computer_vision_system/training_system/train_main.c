...
int main(int argc, char **argv) {
  ...
  if (restore) {
    printf(
        ...
        "to reset)\n");
    if (!ModelCheckpoint(&model, checkpoint_prefix, RESTORE))
      return 1;
  } else {
  ...
  if (!ModelPredict(&model, &testdata[0], 3))
    return 1;

  printf("Training for a few steps\n");
  for (int i = 0; i < 200; ++i) {
    if (!ModelRunTrainStep(&model))
      return 1;
  }
  ...
  if (!ModelCheckpoint(&model, checkpoint_prefix, SAVE))
    return 1;
